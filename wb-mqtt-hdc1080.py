...
class TMQTTHDC1080Sensor(object):
    def init_config(self, fname):
        self.config = json.load(open(fname))

        self.config.setdefault('mqtt_id', 'hdc1080')
        self.config.setdefault('name', 'HDC1080 sensor')
    ...
    def init_sensor(self):
        self.hdc1080 = hdc1080.HDC1080(self.config['i2c_bus'])
    ...
    def start(self):
        ...
        while True:
            try:
                temp, humidity = self.hdc1080.get_value()
            except :
                self.set_channel_error('temperature', "r")
                self.set_channel_error('humidity', "r")
            else:
                self.publish_channel('temperature', temp)
                self.publish_channel('humidity', humidity)
...
def main():
    ...
    sensor = TMQTTHDC1080Sensor(sys.argv[1])
    sensor.start()
